...
namespace KTI.Moo.Extensions.Cyware.App.Dispatcher
{
    ...
    {
        private readonly IBOMVersion<KTI.Moo.Extensions.Cyware.Model.BOMVersion> _bomVersion;
        private readonly IQueueService _queueService;
        private const string QueueName = "moo-cyware-extension-bomversion-dispatcher";

        public BOMVersion(IBOMVersion<Model.BOMVersion> bomVersion, IQueueService queueService)
        {
            _bomVersion = bomVersion;
        ...
        public void Run([QueueTrigger(QueueName, Connection = "AzureWebJobsStorage")] string myQueueItem, int dequeueCount, string Id, string PopReceipt, ILogger log)
        {
            try
            {
                var bomVersion = JsonConvert.DeserializeObject<KTI.Moo.Extensions.Cyware.Model.BOMVersion>(myQueueItem);
                var result = _bomVersion.Upsert(bomVersion);
                log.LogInformation($"C# BOMVersion - Queue trigger function processed: {myQueueItem}");
            }
            catch (Exception ex)
            {
                if (dequeueCount > 5)
