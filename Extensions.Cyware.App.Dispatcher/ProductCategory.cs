...
namespace KTI.Moo.Extensions.Cyware.App.Dispatcher
{
    ...
    {
        private readonly IProductCategory<KTI.Moo.Extensions.Cyware.Model.ProductCategory> _productCategory;
        private readonly IQueueService _queueService;
        private const string QueueName = "moo-cyware-extension-productcategory-dispatcher";
        

        public ProductCategory(IQueueService queueService, IProductCategory<Model.ProductCategory> productCategory)
        {
        ...
        public void Run([QueueTrigger(QueueName, Connection = "AzureWebJobsStorage")] string myQueueItem, int dequeueCount, string Id, string PopReceipt, ILogger log)
        {
            ...
            try
            {
                var productCategory = JsonConvert.DeserializeObject<KTI.Moo.Extensions.Cyware.Model.ProductCategory>(myQueueItem);
                var result = _productCategory.Upsert(productCategory);
                log.LogInformation($"C# ProductCategory - Queue trigger function processed: {myQueueItem}");
            }
            catch (Exception ex)
            {
                if (dequeueCount > 5)
