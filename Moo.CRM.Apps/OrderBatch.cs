...
public class OrderBatch : Helpers.CompanySettings
{
    ...
    private bool ReadFromAzureQueue(QueueClient queueClient, int companyid, ILogger log, int maxmessage = 32)
    {
        ...
        try
        {
            ...
            if (TotalMessage > 0)
            {
                ...
                var ForInsertToOrderList = new List<Model.DTO.Orders.Plugin.Order>();
    ...
    private List<Model.DTO.Orders.Plugin.Order> ReadFromCurrentQueueMessage(List<Model.DTO.Orders.Plugin.Order> ForInsertToOrderList, QueueMessage[] retrievedMessages, ILogger log, int currentcount = 0)
    {
        var TotalIteration = retrievedMessages.Length - 1;
        var orders = Helpers.Decode.Base64(retrievedMessages[currentcount].Body.ToString()).FromBrotliAsync().GetAwaiter().GetResult();

        var NewOrder = JsonConvert.DeserializeObject<Model.DTO.Orders.Plugin.Order>(orders);
    ...
    private bool PostToAPI(List<Model.DTO.Orders.Plugin.Order> ForInsertToOrderList, int CompanyID, ILogger log)
    {
