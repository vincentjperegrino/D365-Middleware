...
{
    public class DiscountTypeHeader : CompanySettings
    {
        private readonly IEmailNotification _emailNotification;
        private readonly IQueueService _queueService;
        private readonly INotification _teamsNotification;

        public DiscountTypeHeader(IEmailNotification emailNotification, IQueueService queueService, INotification teamsNotification)
        {
        ...
        //[FunctionName("DiscountTypeHeader-EmailNotification")]
        public void Run([TimerTrigger(TimerTriggerConfig)] TimerInfo myTimer, ILogger log)
        {
            string className = typeof(DiscountTypeHeader).Name;
            try
            {
                var retrievedMessages = _queueService.RetrieveAndPopMessage(ConnectionString, "moo-cyware-extension-discounttypeheader-dispatcher-poison", ExtensionArchiveQueueName, 32, log);
