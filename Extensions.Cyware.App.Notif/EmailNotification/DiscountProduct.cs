...
namespace KTI.Moo.Extensions.Cyware.App.Queue.EmailNotification
{
    public class DiscountProduct : CompanySettings
    {
        ...
        public DiscountProduct(IEmailNotification emailNotification, IQueueService queueService, INotification teamsNotification)
        {
        ...
        //[FunctionName("DiscountProduct-EmailNotification")]
        public void Run([TimerTrigger(TimerTriggerConfig)] TimerInfo myTimer, ILogger log)
        {
            string className = typeof(DiscountProduct).Name;

            try
            {
                var retrievedMessages = _queueService.RetrieveAndPopMessage(ConnectionString, "moo-cyware-extension-discountproduct-dispatcher-poison", ExtensionArchiveQueueName, 32, log);
