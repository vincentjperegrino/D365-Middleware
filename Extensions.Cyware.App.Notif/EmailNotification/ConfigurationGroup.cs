...
namespace KTI.Moo.Extensions.Cyware.App.Queue.EmailNotification
{
    public class ConfigurationGroup : CompanySettings
    {
        ...
        public ConfigurationGroup(IEmailNotification emailNotification, IQueueService queueService, INotification teamsNotification)
        {
        ...
        //[FunctionName("ConfigurationGroup-EmailNotification")]
        public void Run([TimerTrigger(TimerTriggerConfig)] TimerInfo myTimer, ILogger log)
        {
            string className = typeof(ConfigurationGroup).Name;
            try
            {
                var retrievedMessages = _queueService.RetrieveAndPopMessage(ConnectionString, "moo-cyware-extension-configurationgroup-dispatcher-poison", ExtensionArchiveQueueName, 32, log);
