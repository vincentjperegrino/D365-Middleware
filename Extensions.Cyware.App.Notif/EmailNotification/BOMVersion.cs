...
namespace KTI.Moo.Extensions.Cyware.App.Queue.EmailNotification
{
    public class BOMVersion : CompanySettings
    {
        ...
        public BOMVersion(IEmailNotification emailNotification, IQueueService queueService, INotification teamsNotification)
        {
        ...
        //[FunctionName("BOMVersion-EmailNotification")]
        public void Run([TimerTrigger(TimerTriggerConfig)] TimerInfo myTimer, ILogger log)
        {
            string className = typeof(BOMVersion).Name;
            try
            {
                var retrievedMessages = _queueService.RetrieveAndPopMessage(ConnectionString, "moo-cyware-extension-bomversion-dispatcher-poison", ExtensionArchiveQueueName, 32, log);
