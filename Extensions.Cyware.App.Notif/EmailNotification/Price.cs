...
namespace KTI.Moo.Extensions.Cyware.App.Queue.EmailNotification
{
    public class Price : CompanySettings
    {
        ...
        public Price(IQueueService queueService, IEmailNotification emailNotification, INotification teamsNotification)
        {
        ...
        //[FunctionName("Price-EmailNotification")]
        public void Run([TimerTrigger(TimerTriggerConfig)] TimerInfo myTimer, ILogger log)
        {
            string className = typeof(Price).Name;

            try
            {
                var retrievedMessages = _queueService.RetrieveAndPopMessage(ConnectionString, "moo-cyware-extension-price-dispatcher-poison", ExtensionArchiveQueueName, 32, log);
