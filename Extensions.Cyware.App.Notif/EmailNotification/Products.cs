...
namespace KTI.Moo.Extensions.Cyware.App.Queue.EmailNotification
{
    public class Products : CompanySettings
    {
        ...
        public Products(IQueueService queueService, IEmailNotification emailNotification, INotification teamsNotification)
        {
        ...
        //[FunctionName("Products-EmailNotification")]
        public void Run([TimerTrigger(TimerTriggerConfig)] TimerInfo myTimer, ILogger log)
        {
            string className = typeof(Products).Name;

            try
            {
                var retrievedMessages = _queueService.RetrieveAndPopMessage(ConnectionString, "moo-cyware-extension-productinitial-dispatcher-poison", ExtensionArchiveQueueName, 32, log);
