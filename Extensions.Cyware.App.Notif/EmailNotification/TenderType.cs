...
namespace KTI.Moo.Extensions.Cyware.App.Queue.EmailNotification
{
    public class TenderType : CompanySettings
    {
        ...
        public TenderType(IEmailNotification emailNotification, IQueueService queueService, INotification teamsNotification)
        {
        ...
        //[FunctionName("TenderType-EmailNotification")]
        public void Run([TimerTrigger(TimerTriggerConfig)] TimerInfo myTimer, ILogger log)
        {
            string className = typeof(TenderType).Name;
            try
            {
                var retrievedMessages = _queueService.RetrieveAndPopMessage(ConnectionString, "moo-cyware-extension-tendertype-dispatcher-poison", ExtensionArchiveQueueName, 32, log);
