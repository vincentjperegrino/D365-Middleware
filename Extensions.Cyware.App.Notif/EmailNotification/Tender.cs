...
namespace KTI.Moo.Extensions.Cyware.App.Queue.EmailNotification
{
    public class Tender : CompanySettings
    {
        ...
        public Tender(IEmailNotification emailNotification, IQueueService queueService, INotification teamsNotification)
        {
        ...
        //[FunctionName("Tender-EmailNotification")]
        public void Run([TimerTrigger(TimerTriggerConfig)] TimerInfo myTimer, ILogger log)
        {
            string className = typeof(Tender).Name;
            try
            {
                var retrievedMessages = _queueService.RetrieveAndPopMessage(ConnectionString, "moo-cyware-extension-tender-dispatcher-poison", ExtensionArchiveQueueName, 32, log);
