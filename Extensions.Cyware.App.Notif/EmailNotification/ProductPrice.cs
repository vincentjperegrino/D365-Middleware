...
namespace KTI.Moo.Extensions.Cyware.App.Queue.EmailNotification
{
    public class ProductPrice : CompanySettings
    {
        ...
        public ProductPrice(IQueueService queueService, IEmailNotification emailNotification, INotification teamsNotification)
        {
        ...
        //[FunctionName("ProductPrice-EmailNotification")]
        public void Run([TimerTrigger(TimerTriggerConfig)] TimerInfo myTimer, ILogger log)
        {
            string className = typeof(ProductPrice).Name;
            try
            {
                var retrievedMessages = _queueService.RetrieveAndPopMessage(ConnectionString, "moo-cyware-extension-productprice-dispatcher-poison", ExtensionArchiveQueueName, 32, log);
