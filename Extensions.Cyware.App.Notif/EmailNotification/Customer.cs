...
namespace KTI.Moo.Extensions.Cyware.App.Queue.EmailNotification
{
    public class Customer : CompanySettings
    {
        ...
        public Customer(IEmailNotification emailNotification, IQueueService queueService, INotification teamsNotification)
        {
        ...
        //[FunctionName("Customer-EmailNotification")]
        public void Run([TimerTrigger(TimerTriggerConfig)] TimerInfo myTimer, ILogger log)
        {
            string className = typeof(Customer).Name;
            try
            {
                var retrievedMessages = _queueService.RetrieveAndPopMessage(ConnectionString, "moo-cyware-extension-customer-dispatcher-poison", ExtensionArchiveQueueName, 32, log);
