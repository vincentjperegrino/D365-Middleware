...
namespace KTI.Moo.Extensions.Cyware.App.Queue.EmailNotification
{
    public class BOMHeader : CompanySettings
    {
        ...
        public BOMHeader(IEmailNotification emailNotification, IQueueService queueService, INotification teamsNotification)
        {
        ...
        //[FunctionName("BOMHeader-EmailNotification")]
        public void Run([TimerTrigger(TimerTriggerConfig)] TimerInfo myTimer, ILogger log)
        {
            string className = typeof(BOMHeader).Name;
            try
            {
                var retrievedMessages = _queueService.RetrieveAndPopMessage(ConnectionString, "moo-cyware-extension-bomheader-dispatcher-poison", ExtensionArchiveQueueName, 32, log);
