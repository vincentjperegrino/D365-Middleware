...
{
    public class BOMHeader : KTI.Moo.ChannelApps.Core.Domain.Dispatchers.IBOMHeaderToQueue
    {
        public bool DispatchMessage(string messagequeue, string QueueName, string QueueConnectionString, string CompanyID)
        {
            var bomObject = JsonConvert.DeserializeObject<Extensions.Cyware.Model.BOMHeader>(messagequeue);
