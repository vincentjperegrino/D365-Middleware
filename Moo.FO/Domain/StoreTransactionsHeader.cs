...
{
    public class StoreTransactionsHeader : Base.Domain.IStoreTransactionsHeader
    {
        ...
        public StoreTransactionsHeader(int companyId)
        {
        ...
        public async Task<string> upsert(OrdersTransaction content, ILogger log)
        {
            ...
            var storeTransactionHeader = new StoreTransactioHeaderDTO(content);
            var postModel = JsonConvert.SerializeObject(storeTransactionHeader);
            ...
            while (retryCount <= _retryCounter)
            {
                try
                {
                    ...
                    using (HttpClient httpClient = new HttpClient())
                    {
                        ...
                        HttpResponseMessage retrieveResponseTest = await httpClient.PostAsync("/data/RetailTransactions?cmp=SRDF", new StringContent(postModel, Encoding.UTF8, "application/json"));
                        ...
                        if (!retrieveResponseTest.IsSuccessStatusCode)
                        {
                            ...
                            {
                                throw new Exception("FO Header " + retrieveResponseTest.StatusCode + ": " + retrieveResponseTest.Content.ReadAsStringAsync().Result);
                            }
                ...
                {
                    log.LogInformation("FO Header: " + ex.Message);

                    if (retryCount >= _retryCounter)
                    {
                        log.LogError("FO Header " + ex.Message);
                        throw new Exception("FO Header " + ex.Message);
                    }
        ...
        public async Task<string> upsert(string content, ILogger log)
        {
            ...
            var storeTransactionHeader = JsonConvert.DeserializeObject<StoreTransactionsHeader>(content);
            var postModel = JsonConvert.SerializeObject(storeTransactionHeader);
