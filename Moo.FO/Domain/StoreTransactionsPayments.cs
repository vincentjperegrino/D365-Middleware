...
{
    public class StoreTransactionsPayments : Base.Domain.IStoreTransactionsPayment
    {
        ...
        public StoreTransactionsPayments(int companyId)
        {
        ...
        public async Task<string> upsert(OrdersTransaction content, ILogger log)
        {
            int retryCount = 1;
            int lineNumber = 1;
            foreach (var tenders in content.Tenders)
            {
                ...
                while (retryCount <= _retryCounter)
                {
                    ...
                    try
                    {
                        var details = new StoreTransactionTenderDTO(tenders, content.Config);
                        ...
                        using (HttpClient httpClient = new HttpClient())
                        {
                            ...
                            HttpResponseMessage retrieveResponseTest = await httpClient.PostAsync("/data/RetailTransactionPaymentLinesV2/?cmp=SRDF", new StringContent(postModel, Encoding.UTF8, "application/json"));
                            ...
                            if (!retrieveResponseTest.IsSuccessStatusCode)
                            {
                                ...
                                {
                                    throw new Exception("FO Tenders " + retrieveResponseTest.StatusCode + ": " + retrieveResponseTest.Content.ReadAsStringAsync().Result);
                                }
                    ...
                    {
                        log.LogInformation("FO Tenders: " + ex.Message);

                        if (retryCount >= _retryCounter)
                        {
                            log.LogError("FO Tenders " + ex.Message);
                            throw new Exception("FO Tenders " + ex.Message);
                        }
        ...
        //    int retryCount = 1;
        //    int lineNumber = 1;
        //    foreach (var lines in content.Tenders)
        //    {
        //        while (retryCount <= _retryCounter)
        //        {
        //            try
        //            {
        //                var details = new StoreTransactionLinesDTO(lines, lineNumber);
        //                var postModel = JsonConvert.SerializeObject(details);
        ...
        public async Task<string> upsert(string content, ILogger log)
        {
            ...
            var customer = JsonConvert.DeserializeObject<StoreTransactionsHeader>(content);
