...
{
    public class StoreTransactionsDiscount : Base.Domain.IStoreTransactionsDiscount
    {
        ...
        public StoreTransactionsDiscount(int companyId)
        {
        ...
        public async Task<string> upsert(OrdersTransaction content, ILogger log)
        {
            ...
            foreach (var lines in content.Discounts)
            {
                while (retryCount <= _retryCounter)
                {
                    try
                    {
                        var details = new StoreTransactionDiscountsDTO(lines, content);
                        ...
                        using (HttpClient httpClient = new HttpClient())
                        {
                            ...
                            HttpResponseMessage retrieveResponseTest = await httpClient.PostAsync("/data/RetailTransactionDiscountLines?cmp=SRDF", new StringContent(postModel, Encoding.UTF8, "application/json"));
                            ...
                            if (!retrieveResponseTest.IsSuccessStatusCode)
                            {
                                ...
                                {
                                    throw new Exception("FO Discount " + retrieveResponseTest.StatusCode + ": " + retrieveResponseTest.Content.ReadAsStringAsync().Result);
                                }
                    ...
                    {
                        log.LogInformation("FO Discounts: " + ex.Message);

                        if (retryCount >= _retryCounter)
                        {
                            log.LogError("FO Discounts " + ex.Message);
                            throw new Exception("FO Discounts " + ex.Message);
                        }
