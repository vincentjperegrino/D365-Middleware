...
namespace KTI.Moo.ChannelApps.Cyware.App.Queue.EmailNotification
{
    public class ForEx : CompanySettings
    {
        ...
        public ForEx(IEmailNotification emailNotification, IQueueService queueService, INotification teamsNotification)
        {
        ...
        //[FunctionName("ForEx-EmailNotification")]
        ...
        {
            string className = typeof(ForEx).Name;

            try
            {
                //var retrievedMessages = _queueService.RetrieveAndPopMessage(ConnectionString, "rdf-cyware-channelapp-forex-dispatcher-poison", 32, log);
                var retrievedMessages = _queueService.RetrieveAndPopMessage(ConnectionString, "test-forex-poison", ChannelArchiveQueueName, 32, log);
