...
namespace KTI.Moo.ChannelApps.Lazada.Implementations.NCCI;

public class Order_Plugin : Core.Domain.IOrder<ChannelApps.Model.NCCI.Receivers.Plugin.Order, ChannelApps.Model.NCCI.Receivers.Plugin.Customer, ChannelApps.Model.NCCI.Receivers.Invoice>, IOrderForReceiver
{
    private readonly string _connectionString;
    private readonly string _companyId;

    public Order_Plugin(string connectionString, string companyId)
    {
    ...
    public List<Model.NCCI.Receivers.Plugin.OrderItem> GetClientModelOrderItem(string decodedJsonString)
    {
        ...
        var linenumber = 1;
        var OrderItemModel = channelAppsInvoice.order.order_items.Select(orderitem =>
        {

            var OrderItemModel = new Model.NCCI.Receivers.Plugin.OrderItem(orderitem);
            OrderItemModel.kti_sourceid = channelAppsInvoice.order.kti_sourceid;
