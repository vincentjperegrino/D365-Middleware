...
namespace KTI.Moo.ChannelApps.OctoPOS.App.Dispatcher.PoisonQueue;
...
public class Customer : CompanySettings
{
    ...
    public void Run([QueueTrigger("%CompanyID%-octopos-%StoreCode%-channelapp-customer-dispatcher-poison", Connection = "AzureQueueConnectionString")] string myQueueItem, ILogger log)
    {
        var webhookUrl = Environment.GetEnvironmentVariable("TeamsNotificationWebHook");
        var Title = $"{Companyid}>NCCI>{StoreCode}>Dispatcher>Octopus>Customer>Error Alert";
