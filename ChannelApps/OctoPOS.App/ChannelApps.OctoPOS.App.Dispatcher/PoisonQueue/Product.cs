...
public class Product: CompanySettings
{
    ...
    public void Run([QueueTrigger("%CompanyID%-octopos-%StoreCode%-channelapp-product-dispatcher-poison", Connection = "AzureQueueConnectionString")] string myQueueItem, ILogger log)
    {
        var webhookUrl = Environment.GetEnvironmentVariable("TeamsNotificationWebHook");
        var Title = $"{Companyid}>NCCI>{StoreCode}>Dispatcher>Octopus>Product>Error Alert";
